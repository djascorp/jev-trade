...
Walk-forward validated (5-fold, 9000 candles):
  R_25 M15 MACD + Strength>p75 + 20min = 64.0% WR (p=0.0053, σ=14.7)
Previous (no strength filter): 51.8% WR OOS (p=0.24, random)
"""
import sys, os, time
...
class MACDBot(BaseBot):
    """Trade with MACD signal line crossover + signal strength filter."""
    ...
    def __init__(self, config: dict):
        ...
        self.strength_lookback = config.get("strength_lookback", 200)  # rolling window
        self.strength_percentile = config.get("strength_percentile", 75)  # p75
        self.strength_history = deque(maxlen=self.strength_lookback)
        self.strength_filtered = 0  # counter for logging
    ...
    async def _on_candle_close(self, candle: dict):
        ...
        macd_diff = macd_line[i] - signal_line[i]
        abs_diff = abs(macd_diff)
        ...
        if signal:
            ...
            if len(self.strength_history) >= 50:  # need enough history
